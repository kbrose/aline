function initMap() {
  ...
  var map = L.map("map");
  L.tileLayer(
    `https://maps.geoapify.com/v1/tile/osm-bright/{z}/{x}/{y}.png?apiKey=${x}`,
    {
      attribution:
  ...
  function onLocationError(e) {
    alert(
      "Unable to get your location. Make sure it is enabled in your settings."
    );
  }
  ...
  document.getElementById("link-input-go").addEventListener("click", () => {
    overlayButton(map);
  });
...
function setOverlayOpacity(alpha) {
  document.getElementById("overlay-canvas").getContext("2d").globalAlpha =
    alpha;
}
...
function overlayMap(mapUrl, leafletMap) {
  function makeOverlay(leafletMap, toOverlay, imgHeight, imgWidth) {
    ...
    if (imgHeight / imgWidth > mapPixelHeight / mapPixelWidth) {
      ...
      bottomRight = leafletMap.containerPointToLatLng([
        width + widthOffset,
        height + heightOffset,
      ]);
      bottomLeft = leafletMap.containerPointToLatLng([
        widthOffset,
        height + heightOffset,
      ]);
      topRight = leafletMap.containerPointToLatLng([
        width + widthOffset,
        heightOffset,
      ]);
  ...
  if (isPdfFile(mapUrl)) {
    overlayPromise = pdfjsLib.getDocument(mapUrl).promise.then(function (pdf) {
      ...
      if (pdf.numPages > 1) {
        pageNum = parseInt(
          window.prompt("Which page of the PDF should be used?")
        );
        while (isNaN(pageNum)) {
          pageNum = parseInt(
            window.prompt(
              "Sorry, I require a number.\nWhich page of the PDF should be used?"
            )
          );
        }
      }
    ...
    overlayPromise = loadImage(mapUrl).then(function (img) {
